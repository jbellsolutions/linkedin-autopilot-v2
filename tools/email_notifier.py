...
def _send_via_resend(api_key: str, from_email: str, to_email: str,
    ...
    try:
        resp = requests.post(
            ...
            json={
                "from": f"LinkedIn Autopilot <{from_email}>",
                "to": [to_email],
                "subject": subject,
                "text": text,
                "html": html,
...
def _build_email(result: dict, date_str: str) -> tuple[str, str]:
    ...
    if status == "success":
        if counts["new_format"]:
            ...
            subject = f"Content Ready — {date_str} — {', '.join(parts)}"
        else:
            subject = f"Content Ready — {date_str} — {counts['total_approved']} approved"
    elif status == "error":
        subject = f"Content FAILED — {date_str}"
    else:
        subject = f"Content — {date_str} — {status}"
    ...
    lines.append("— LinkedIn Autopilot (Automated)")
...
def _build_html_email(result: dict, date_str: str) -> str:
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
        <h1 style="font-size: 24px; margin-bottom: 5px;">Content Pipeline</h1>
    ...
    html_parts.append(f"""
        ...
        <p style="color: #999; font-size: 12px; margin-top: 10px;">LinkedIn Autopilot (Automated)</p>
    </div>
    """)

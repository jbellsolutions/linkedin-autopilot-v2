...
def _generate_intense_pillow(
    ...
    # Author bar at bottom — reads from config if available
    _biz = {}
    try:
        from agents.base import BaseAgent
        _biz = BaseAgent._load_yaml("business.yaml").get("business", {})
    except Exception:
        pass
    author_name = _biz.get("owner_name", "").upper() or ""
    brand_text = _biz.get("brand", "") or ""
    author_font = get_font(18, bold=True)
    brand_font = get_font(15, bold=False)
    if author_name:
        draw.text((x_margin, height - 65), author_name, fill=(200, 200, 200), font=author_font)
    if brand_text:
        draw.text((x_margin, height - 42), brand_text, fill=(120, 120, 120), font=brand_font)

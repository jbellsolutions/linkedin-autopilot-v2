"""LinkedIn Autopilot — Main Entry Point
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("linkedin-autopilot.log"),
    ],
)
...
def main():
    parser = argparse.ArgumentParser(description="LinkedIn Autopilot")
    parser.add_argument(
        "--run",
        type=str,
    ...
    config_path = os.path.join(PROJECT_DIR, "config", "business.yaml")
    if not os.path.exists(config_path):
        logger.warning(
            "config/business.yaml not found. "
            "Copy config_examples/business.yaml to config/business.yaml and edit it, "
            "or run 'python setup.py' to configure."
        )

    logger.info("LinkedIn Autopilot starting...")
    check_env()
